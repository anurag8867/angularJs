        ...
        client.on('command', (command, args) => {
            switch (command) {
                ...
                case 'leave':
                    ...
                    break;
                default:
                    this.emit('response', `Unknown command..., Please type 'help' to know the supported commands`);
            }
        });
    }
...
leave :{car_number}` );
    ...
    create_parking_lot(args) {
        if (!args[0]) {
            this.emit('response', `Need a size to create a parking lot`);
            return;
        }
        if (parseInt(args[0]) === 0) {
            this.emit('response', `Parking lot size must be greater than 0`);
            return;
        }
        this.parkingLotSize = args[0];
        this.emit('response', `Created parking lot with ${this.parkingLotSize} slots`);
    ...
    park(args) {
        if (!this.parkingLotSize) {
            this.emit('response', `No parking slot created, please create a parking slot first with 'create_parking_lot' command`);
            return;
        }
        let slotNumber = this.getCurrentAvailableSlotNumber()
        if (!slotNumber) {
            this.emit('response', `Sorry, parking lot is full`);
            return;
        }
        if (!args[0]) {
            this.emit('response', `Need a valid registration number of a car`);
            return;
        }
        //Making sure this car is not already parked in our lot
        if (this.getSlotNumberFromRegistrationNumber(args[0])) {
            this.emit('response', `The car with registration number ${args[0]} is already parked with us at ` +
                `${this.getSlotNumberFromRegistrationNumber(args[0])} slot number`);
            return;
        }
        this.slots.set(slotNumber, args[0]);
        this.emit('response', `Allocated slot number: ${slotNumber}`);
    }
     ...
     */
    status() {
        let parkedCars = getParkedCars(this.slots);
        if (!parkedCars) {
            this.emit('response', `No Car parked in the lot`);
            return;
        }
        this.emit('response', `Slot No. Registration No.\n${parkedCars}`);
    }
    ...
    */
    leave(args) {
        ...
        let slotNumber = this.getSlotNumberFromRegistrationNumber(args[0]);
        //Making sure this car is parked in our lot
        if (!slotNumber) {
            this.emit('response', `Registration number ${args[0]} not found`);
            return;
        }
        if (!args[1]) {
            this.emit('response', `Need total parked hours to calculate parking charge, 1 is min hour to be charged`);
            return;
        }
        let charge = calculateParkingCharge(args[1]);
        this.slotAvailabilityQueue.push(slotNumber);
        this.slots.set(slotNumber, false);
        this.emit('response', `Registration number ${args[0]} with Slot Number ${slotNumber} is free with Charge ${charge}`);
        return;
    }
    ...
    /**
     * Taking out the slot number whose car is about to leave
     * @param {*} carNumber 
     */
    getSlotNumberFromRegistrationNumber(carNumber) {
        for (let [key, value] of this.slots) {
            if (carNumber === value) {
                return key;
            }
 ...
 * @param {*} hour 
 */
function calculateParkingCharge(hour) {
 ...
 * @param {} slots 
 */
function getParkedCars(slots) {
    let parkedCars = [];
    for (let [key, value] of slots) {
        parkedCars.push(`${key}: ${value === false ? "Empty" : value}`);
    }
